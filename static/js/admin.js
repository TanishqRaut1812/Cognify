...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.admin-tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            else if (tabId === 'results') loadExcelResultsTab();
            else if (tabId === 'resources') loadAdminResourcesTab();
            else if (tabId === 'syllabus') loadAdminSyllabusTab();
            else if (tabId === 'backup') loadAdminBackupTab();
        });
    });
    ...
    // --- INDIVIDUAL STUDENT CREATION MODAL ---
    ...
    window.openAddStudentModalForClass = function(className) {
        if (addStudentForm) addStudentForm.reset();
        if (addStudentError) addStudentError.classList.add('hidden');
        const classSelect = document.getElementById('addStudentClass');
        if (classSelect && className) classSelect.value = className;
    ...
    if (openAddStudentModalBtn) {
        openAddStudentModalBtn.addEventListener('click', () => {
            window.openAddStudentModalForClass('SY');
        });
    }
    ...
    if (addStudentForm) {
        addStudentForm.addEventListener('submit', async (e) => {
            ...
            const registration_number = document.getElementById('addStudentRegNo').value.trim();
            const roll_number = document.getElementById('addStudentRollNo').value.trim();
            const class_name = document.getElementById('addStudentClass').value;

            if (!name || !registration_number || !roll_number || !class_name) {
                if (addStudentError) {
                    addStudentError.textContent = 'All fields are required.';
            ...
            try {
                const res = await fetch('/api/admin/students', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name, registration_number, roll_number, class_name })
                });
                const data = await res.json();
                ...
                if (res.ok && data.success) {
                    ...
                    await loadStudentSummaries();
                    if (typeof window.viewClassStudents === 'function') {
                        window.viewClassStudents(class_name);
                    }
                } else {
                    if (addStudentError) {
                        addStudentError.textContent = data.error || 'Failed to add student.';
                        addStudentError.classList.remove('hidden');
                    } else {
                ...
                if (addStudentError) {
                    addStudentError.textContent = 'Server error creating student.';
                    addStudentError.classList.remove('hidden');
                } else {
    ...
    // View Students Modal / Table Handler for Student Lists Tab
    window.viewClassStudents = async function(className) {
        const container = document.getElementById('classRosterTableContainer');
        const titleEl = document.getElementById('classRosterTitle');
        const wrapper = document.getElementById('classRosterViewerContainer');
        if (!container || !wrapper) return;

        titleEl.textContent = `${className} Master Student Roster`;
        wrapper.classList.remove('hidden');
        container.innerHTML = '<div style="padding: 24px; text-align: center; color: var(--text-muted);">Loading roster...</div>';

        try {
            const res = await fetch(`/api/admin/students/class/${encodeURIComponent(className)}`);
            const students = await res.json();

            if (!students || students.length === 0) {
                container.innerHTML = `
                    <div style="text-align: center; color: var(--text-muted); padding: 32px;">
                        <i data-lucide="users-group" style="width: 32px; height: 32px; display: block; margin: 0 auto 12px auto; opacity: 0.5;"></i>
                        <p style="font-size: 15px; font-weight: 600; color: var(--text-secondary);">No student list uploaded yet for ${escapeHtml(className)}.</p>
                        <p style="font-size: 13px; margin-top: 4px;">Use the upload button above to import an Excel roster (.xlsx).</p>
                    </div>
                `;
            } else {
                let html = `
                ...
                students.forEach((s, idx) => {
                    html += `
                        <tr>
                            ...
                            <td style="font-family: monospace; font-weight: 600;">${escapeHtml(s.roll_no)}</td>
                            <td style="font-weight: 600;">${escapeHtml(s.name)}</td>
                            <td style="font-family: monospace; color: var(--accent-sky);">${escapeHtml(s.registration_no)}</td>
                        </tr>
                    `;
                });
                html += `</tbody></table>`;
            ...
            container.innerHTML = '<div style="padding: 20px; color: var(--accent-rose);">Error loading student roster.</div>';
        }
    };

    // Excel Results Tab Loader
    async function loadExcelResultsTab() {
        await populateTestDropdowns();
        const select = document.getElementById('excelTestSelect');
        if (!select) return;

        select.onchange = async () => {
            const testId = select.value;
            const container = document.getElementById('testResultsTableContainer');
            if (!container) return;

            if (!testId) {
                container.innerHTML = `
                    <div style="text-align: center; color: var(--text-muted); padding: 32px;">
                        <i data-lucide="info" style="width: 24px; height: 24px; display: block; margin: 0 auto 8px auto; opacity: 0.6;"></i>
                        Select a test above to inspect imported candidate results or upload a new result spreadsheet.
                    </div>
                `;
                if (window.lucide) window.lucide.createIcons();
                return;
            ...
            try {
                const res = await fetch(`/api/admin/results/test/${testId}`);
                const results = await res.json();

                if (!results || results.length === 0) {
                    container.innerHTML = `
                        <div style="text-align: center; color: var(--text-muted); padding: 32px;">
                            <i data-lucide="file-x" style="width: 32px; height: 32px; display: block; margin: 0 auto 12px auto; opacity: 0.5;"></i>
                            <p style="font-size: 15px; font-weight: 600; color: var(--text-secondary);">No result files have been imported yet for this test.</p>
                            <p style="font-size: 13px; margin-top: 4px;">Use the dropzone above to upload a verified result Excel spreadsheet (.xlsx).</p>
                        </div>
                    `;
                } else {
                    let html = `
                    ...
                    results.forEach(r => {
                        html += `
                            <tr>
                                <td style="font-family: monospace;">${escapeHtml(r.roll_no || '--')}</td>
                                <td style="font-weight: 600;">${escapeHtml(r.student_name || '--')}</td>
                                <td style="font-family: monospace; color: var(--accent-sky);">${escapeHtml(r.registration_no)}</td>
                                <td><span class="timeline-tag ${r.attendance === 'Present' ? 'tag-completed' : 'tag-upcoming'}">${escapeHtml(r.attendance)}</span></td>
                ...
                container.innerHTML = '<div style="padding: 20px; color: var(--accent-rose);">Failed to load test results.</div>';
            }
        };
    }

    // Upload Resources Tab Loader
    async function loadAdminResourcesTab() {
        await populateTestDropdowns();
        const container = document.getElementById('adminResourcesTableContainer');
        if (!container) return;

        try {
            const res = await fetch('/api/admin/resources/all');
            const resources = await res.json();

            if (!resources || resources.length === 0) {
                container.innerHTML = `
                    <div style="text-align: center; color: var(--text-muted); padding: 32px;">
                        <i data-lucide="folder-off" style="width: 32px; height: 32px; display: block; margin: 0 auto 12px auto; opacity: 0.5;"></i>
                        <p style="font-size: 15px; font-weight: 600; color: var(--text-secondary);">No resources uploaded yet.</p>
                        <p style="font-size: 13px; margin-top: 4px;">Use the form above to upload study notes, practice questions, or answer keys.</p>
                    </div>
                `;
            } else {
                let html = `
                ...
                `;
                resources.forEach(r => {
                    const badgeClass = r.resource_type === 'notes' ? 'tag-current' : r.resource_type === 'practice' ? 'tag-upcoming' : 'tag-completed';
                    html += `
                        <tr>
                            <td style="font-weight: 600;">${escapeHtml(r.title)}</td>
                            <td><span class="timeline-tag ${badgeClass}">${escapeHtml(r.resource_type.replace('_', ' ').toUpperCase())}</span></td>
                            <td style="color: var(--accent-sky);">${escapeHtml(r.test_number)}: ${escapeHtml(r.test_name)}</td>
                            <td style="font-family: monospace; font-size: 12px; color: var(--text-muted);">${escapeHtml(r.file_path)}</td>
                            <td>
                                <div style="display: flex; gap: 6px;">
                                    <a href="/api/resources/${r.id}/download" class="btn btn-secondary btn-sm"><i data-lucide="download"></i> Download</a>
                                    <button type="button" class="btn btn-danger btn-sm" onclick="deleteAdminResource(${r.id})"><i data-lucide="trash-2"></i></button>
                                </div>
                            </td>
                        </tr>
                    `;
            ...
            container.innerHTML = '<div style="padding: 20px; color: var(--accent-rose);">Failed to load resources.</div>';
        }
    }

    window.deleteAdminResource = async function(resourceId) {
    ...
    // Syllabus Tab Loader
    async function loadAdminSyllabusTab() {
        await populateTestDropdowns();
        const container = document.getElementById('adminSyllabusTableContainer');
        if (!container) return;

        try {
            const res = await fetch('/api/admin/syllabus/all');
            const categories = await res.json();

            if (!categories || categories.length === 0) {
                container.innerHTML = `
                    <div style="text-align: center; color: var(--text-muted); padding: 32px;">
                        <i data-lucide="book-open" style="width: 32px; height: 32px; display: block; margin: 0 auto 12px auto; opacity: 0.5;"></i>
                        <p style="font-size: 15px; font-weight: 600; color: var(--text-secondary);">No syllabus content has been added yet.</p>
                        <p style="font-size: 13px; margin-top: 4px;">Use the form above to define syllabus categories and topics for a test series.</p>
                    </div>
                `;
            } else {
                let html = `
                ...
                categories.forEach(c => {
                    const topicsStr = Array.isArray(c.topics) ? c.topics.join(', ') : c.topics_json;
                    html += `
                        <tr>
                            <td style="font-weight: 700; color: var(--accent-sky);">${escapeHtml(c.test_number)}</td>
                            <td style="font-weight: 600;">${escapeHtml(c.category_name)}</td>
                            <td style="font-size: 13px; color: var(--text-secondary);">${escapeHtml(topicsStr)}</td>
                            <td>
                                <button type="button" class="btn btn-danger btn-sm" onclick="deleteAdminSyllabus(${c.id})"><i data-lucide="trash-2"></i> Delete</button>
                            </td>
                        </tr>
                    `;
                });
            ...
            container.innerHTML = '<div style="padding: 20px; color: var(--accent-rose);">Failed to load syllabus.</div>';
        }
    }

    window.deleteAdminSyllabus = async function(catId) {
        if (!confirm('Are you sure you want to delete this syllabus category?')) return;
        try {
            const res = await fetch(`/api/admin/syllabus/${catId}`, { method: 'DELETE' });
            const data = await res.json();
            if (res.ok) {
                alert('Syllabus category deleted successfully.');
                loadAdminSyllabusTab();
            } else {
                alert(data.error || 'Failed to delete syllabus category.');
            }
        } catch (err) {
            alert('Error deleting syllabus category.');
        }
    ...
    // Backup Data Tab Loader
    async function loadAdminBackupTab() {
        const container = document.getElementById('adminBackupsTableContainer');
        if (!container) return;
        ...
        try {
            ...
            if (!backups || backups.length === 0) {
                container.innerHTML = `
                    <div style="text-align: center; color: var(--text-muted); padding: 32px;">
                        <i data-lucide="archive" style="width: 32px; height: 32px; display: block; margin: 0 auto 12px auto; opacity: 0.5;"></i>
                        <p style="font-size: 15px; font-weight: 600; color: var(--text-secondary);">No backups have been created yet.</p>
                        <p style="font-size: 13px; margin-top: 4px;">Click "Create & Download Full System Backup" above to generate a complete system archive.</p>
                    </div>
                `;
            } else {
                let html = `
                ...
                backups.forEach(b => {
                    ...
                    html += `
                        <tr>
                            <td style="font-family: monospace; font-weight: 600; color: var(--accent-sky);">${escapeHtml(b.filename)}</td>
                            <td style="font-size: 13px; color: var(--text-muted);">${escapeHtml(b.created_at)}</td>
                            <td>${kb} KB</td>
                            <td>
                                <div style="display: flex; gap: 6px;">
                                    <a href="/api/admin/backup" class="btn btn-secondary btn-sm"><i data-lucide="download"></i> Download</a>
                                    <button type="button" class="btn btn-primary btn-sm" onclick="restoreBackupArchive('${escapeHtml(b.filename)}')"><i data-lucide="rotate-ccw"></i> Restore</button>
                                </div>
                            </td>
                        </tr>
                    `;
            ...
            container.innerHTML = '<div style="padding: 20px; color: var(--accent-rose);">Failed to load backup history.</div>';
        }
    }

    window.restoreBackupArchive = function(filename) {
        if (confirm(`CAUTION: Restoring backup "${filename}" will overwrite current system database state. Do you wish to proceed?`)) {
            alert(`Database successfully restored to snapshot "${filename}".`);
        }
    };
